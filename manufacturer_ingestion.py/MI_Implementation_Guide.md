...
- Default: LlamaParse to JSON structure containing:
  - pages (text with spans + bbox), 
  - tables (as matrix + bbox), 
  - images (extracted + bbox), 
  - hyperlinks, 
  - headings.
...
class ParsedElement(BaseModel):
    element_type: str  # 'heading', 'text', 'table', 'image', 'chart'
...
class ParsedPage(BaseModel):
    ...
    parsing_mode: str = ""
    triggered_auto_mode: bool = False

class LlamaParseProcessor:
    def __init__(self, api_key: str, config: dict):
        self.parser = LlamaParse(
            api_key=api_key,
            result_type="json",  # Use JSON for structured output
            auto_mode=True,
            auto_mode_trigger_on_image_in_page=True,
            auto_mode_trigger_on_table_in_page=True,
            annotate_links=True,
            premium_mode=config.get('premium_mode', True),
            parsing_instruction=config.get('parsing_instruction', self._get_default_parsing_instruction())
        )
        self.fallback_enabled = config.get('enable_fallback', True)
    
    def _get_default_parsing_instruction(self) -> str:
        return """
        You are parsing a technical manual for laboratory instruments. 
        Extract and preserve all structural information including:
        - Section headings and their hierarchy levels
        - Table structures with accurate cell relationships
        - Figure and image captions with references
        - Procedure steps with proper numbering
        - Technical specifications and parameter tables
        - Safety warnings and notes (mark these clearly)
        - Cross-references and hyperlinks
        - Page numbers and document structure
        
        For tables: Preserve exact structure, headers, and data relationships.
        For images: Extract any visible text, captions, and technical details.
        For procedures: Maintain step numbering and hierarchical organization.
        """
    
    async def process_document(self, file_path: str) -> Dict[str, Any]:
        """Process PDF through LlamaParse with structured output"""
        try:
            # Primary processing with LlamaParse
            ...
            if not result or not result.pages:
                if self.fallback_enabled:
                    ...
                    raise Exception("No pages extracted from document")
            ...
            for page_data in result.pages:
                parsed_page, page_elements, page_tables, page_images = self._process_page(page_data)
                parsed_pages.append(parsed_page)
                all_elements.extend(page_elements)
                document_tables.extend(page_tables)
                document_images.extend(page_images)
            
            return {
                "success": True,
                "pages": [page.dict() for page in parsed_pages],
                ...
                "document_metadata": {
                    ...
                    "total_images": len(document_images),
                    "parsing_modes_used": list(set(page.parsing_mode for page in parsed_pages))
                }
            }
    ...
    def _process_page(self, page_data) -> tuple:
        """Process individual page data from LlamaParse result"""
        ...
        # Process elements (items)
        elements = []
        if hasattr(page_data, 'items') and page_data.items:
            for item in page_data.items:
                element = ParsedElement(
                    ...
                    bbox={
                        ...
                        "width": item.bBox.w,
                        "height": item.bBox.h
                    } if hasattr(item, 'bBox') and item.bBox else None,
        ...
        if hasattr(page_data, 'images') and page_data.images:
            for idx, image in enumerate(page_data.images):
                image_info = {
                    "image_id": f"page_{page_number}_image_{idx}",
                    "page_number": page_number,
                    "bbox": {
                        "x": image.bbox.x,
                        "y": image.bbox.y,
                        "width": image.bbox.w, 
                        "height": image.bbox.h
                    } if hasattr(image, 'bbox') and image.bbox else None,
                    "extracted_text": getattr(image, 'text', ''),
                    "image_type": getattr(image, 'type', 'unknown'),
        ...
        # Process tables
        page_tables = []
        if hasattr(page_data, 'tables') and page_data.tables:
            for idx, table in enumerate(page_data.tables):
                table_info = {
                    ...
                    "bbox": {
                        ...
                        "width": table.bbox.w,
                        "height": table.bbox.h
                    } if hasattr(table, 'bbox') and table.bbox else None,
                    "rows": getattr(table, 'rows', []),
                    "markdown": getattr(table, 'md', ''),
                    "csv_data": getattr(table, 'csv', ''),
        ...
        # Process charts
        page_charts = []
        if hasattr(page_data, 'charts') and page_data.charts:
            for idx, chart in enumerate(page_data.charts):
                chart_info = {
                    "chart_id": f"page_{page_number}_chart_{idx}",
                    "page_number": page_number,
                    "bbox": {
                        "x": chart.bbox.x,
                        "y": chart.bbox.y,
                        "width": chart.bbox.w,
                        "height": chart.bbox.h
                    } if hasattr(chart, 'bbox') and chart.bbox else None,
                    "extracted_text": getattr(chart, 'text', ''),
                    "chart_type": getattr(chart, 'type', 'unknown'),
                    "base64_data": getattr(chart, 'base64', ''),
        ...
        # Process links
        links = []
        if hasattr(page_data, 'links') and page_data.links:
            links = [
        ...
        parsed_page = ParsedPage(
            ...
            charts=page_charts,
            links=links,
            parsing_mode=getattr(page_data, 'parsingMode', ''),
            triggered_auto_mode=getattr(page_data, 'triggeredAutoMode', False)
        )
        
        return parsed_page, elements, page_tables, page_images
    
    def get_page_nodes(self, parsed_pages: List[ParsedPage], separator: str = "\n---\n") -> List[TextNode]:
        """Split each page into TextNode for further processing"""
        nodes = []
        
        for page in parsed_pages:
            # Create node for full page
            page_chunks = page.text.split(separator) if separator in page.text else [page.text]
                ...
                if chunk.strip():  # Skip empty chunks
                    node = TextNode(
                        ...
                        metadata={
                            ...
                            "parsing_mode": page.parsing_mode,
                            "triggered_auto_mode": page.triggered_auto_mode,
    ...
    def extract_structured_elements(self, parsed_pages: List[ParsedPage]) -> Dict[str, List]:
        """Extract and organize structured elements by type"""
        ...
        for page in parsed_pages:
            ...
            for element in page.elements:
                if element.element_type == 'heading':
                    ...
                    if self._is_procedure_text(element.content):
            ...
            structured_data["images"].extend(page.images)
            structured_data["charts"].extend(page.charts)
            structured_data["links"].extend([
                {**link, "page": page.page_number} for link in page.links
            ])
        
        return structured_data
    
    def _is_procedure_text(self, text: str) -> bool:
        """Check if text contains procedural steps"""
        import re
        
        # Look for numbered steps, bullet points, or procedure keywords
        step_patterns = [
            r'^\s*\d+\.',  # "1. Step one"
            r'^\s*Step \d+',  # "Step 1:"
            r'^\s*\(\d+\)',  # "(1) First step"
            r'^\s*[a-zA-Z]\.',  # "a. First step"
        ]
        
        procedure_keywords = [
            'procedure', 'protocol', 'method', 'instructions',
            'calibration', 'maintenance', 'operation'
        ]
        ...
        for line in lines:
            if any(re.match(pattern, line.strip()) for pattern in step_patterns):
                step_count += 1
        
        has_keywords = any(keyword in text.lower() for keyword in procedure_keywords)
        
        return step_count >= 2 or (step_count >= 1 and has_keywords)
    
    async def _fallback_processing(self, file_path: str) -> Dict[str, Any]:
        """Fallback processing using Unstructured + Camelot/Tabula"""
        try:
            ...
            for page_num, texts in page_texts.items():
                page = ParsedPage(
                    ...
                    text='\n'.join(texts),
                    markdown='\n'.join(texts),  # Basic conversion
                    width=612,  # Default PDF width
                    height=792,  # Default PDF height
                    elements=[],
                    parsing_mode="fallback_unstructured"
                )
                pages.append(page)
            
            # Process tables from Camelot
            processed_tables = []
            for table in tables:
                table_info = {
                    "table_id": f"fallback_table_{table.page}_{len(processed_tables)}",
                    "page_number": table.page,
                    "rows": table.df.values.tolist(),
                    "markdown": table.df.to_markdown(),
                    "csv_data": table.df.to_csv(),
                    "metadata": {"extraction_method": "camelot"}
                }
            ...
            return {
                ...
                "images": [],  # Unstructured doesn't extract images easily
                "document_metadata": {
...
- Target 1000–1200 tokens; keep tables as Markdown fenced blocks.
...
- Caption tables/images; summary(≤2 sentences) per chunk; tags (top-k keywords); optional context_label (e.g., “Error codes”, “Parameters”).
